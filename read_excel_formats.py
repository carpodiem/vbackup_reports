...
TEMPLATE_FILE = "templates/backup_size_23-03.xlsx"


def get_color(workbook, color):
    if color.type == 'rgb':
        return color.rgb
    elif color.type == 'indexed':
        return openpyxl.styles.colors.COLOR_INDEX[color.indexed]
    elif color.type == 'theme':
        return workbook.theme_elements.clrScheme.srgbClr_lst[color.theme].val

def read_cell_styles(worksheet, cell_address):
...
if __name__ == "__main__":
    ...
    cell_styles = read_cell_styles(worksheet, 'A1')

    # Выведите стили на экран
    for style_name, style_value in cell_styles.items():
